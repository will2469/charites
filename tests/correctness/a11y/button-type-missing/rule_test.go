...
func TestRule_A11yButtonTypeMissing_TriCorpus(t *testing.T) {
	rule := a11y.NewButtonTypeMissingRule()
	baseDir := "."
	if _, err := os.Stat("positive"); os.IsNotExist(err) {
		baseDir = filepath.Join("tests", "correctness", "a11y", "button-type-missing")
	}
