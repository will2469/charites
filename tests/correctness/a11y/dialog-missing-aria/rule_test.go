...
func TestRule_A11yDialogMissingAria_TriCorpus(t *testing.T) {
	rule := a11y.NewDialogMissingAriaRule()
	baseDir := "."
	if _, err := os.Stat("positive"); os.IsNotExist(err) {
		baseDir = filepath.Join("tests", "correctness", "a11y", "dialog-missing-aria")
	}
