...
func TestRule_A11yEmptyInteractive_TriCorpus(t *testing.T) {
	rule := a11y.NewEmptyInteractiveRule()
	baseDir := "."
	if _, err := os.Stat("positive"); os.IsNotExist(err) {
		baseDir = filepath.Join("tests", "correctness", "a11y", "empty-interactive")
	}
