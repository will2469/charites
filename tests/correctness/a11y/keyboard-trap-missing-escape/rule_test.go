...
func TestRule_A11yKeyboardTrapMissingEscape_TriCorpus(t *testing.T) {
	rule := a11y.NewKeyboardTrapMissingEscapeRule()
	baseDir := "."
	if _, err := os.Stat("positive"); os.IsNotExist(err) {
		baseDir = filepath.Join("tests", "correctness", "a11y", "keyboard-trap-missing-escape")
	}
