...
func TestComputeSpecificity_W3CSpec(t *testing.T) {
	tests := []struct {
		name     string
		selector string
		expected theme.Specificity
	}{
		// 1. Basic selectors & universals
		{
			name:     "Universal selector has zero specificity",
			selector: "*",
			expected: theme.Specificity{A: 0, B: 0, C: 0},
		},
		{
			name:     "Type/element selector",
			selector: "div",
			expected: theme.Specificity{A: 0, B: 0, C: 1},
		},
		{
			name:     "Class selector",
			selector: ".card",
			expected: theme.Specificity{A: 0, B: 1, C: 0},
		},
		{
			name:     "ID selector",
			selector: "#header",
			expected: theme.Specificity{A: 1, B: 0, C: 0},
		},
		{
			name:     "Compound selector: element + class + ID",
			selector: "div.card#header",
			expected: theme.Specificity{A: 1, B: 1, C: 1},
		},
		{
			name:     "Complex combinator hierarchy",
			selector: "body > div.main p + span.icon",
			expected: theme.Specificity{A: 0, B: 2, C: 4}, // body, div, p, span (C=4); .main, .icon (B=2)
		},
		...
		},
		{
			...
			selector: "html.dark",
			expected: theme.Specificity{A: 0, B: 1, C: 1},
		},
		{
		...
		},
		{
			...
			expected: theme.Specificity{A: 0, B: 0, C: 2}, // input + ::placeholder
		},
		{
			name:     "Legacy CSS2 single-colon pseudo-element :before",
			selector: "p:before",
			expected: theme.Specificity{A: 0, B: 0, C: 2}, // p + :before
		},
		{
		...
		},
		{
			...
			expected: theme.Specificity{A: 1, B: 0, C: 0}, // #main wins (1, 0, 0)
		},
		{
		...
		},
		{
			...
			expected: theme.Specificity{A: 0, B: 1, C: 0}, // .icon (0, 1, 0) > span (0, 0, 1)
		},
		{
			name:     "Complex selector with :where and :not",
			selector: "ul:where(#sidebar) li:not(.active)",
			expected: theme.Specificity{A: 0, B: 1, C: 2}, // ul(C=1) + where(0) + li(C=1) + not(.active)(B=1)
		},
		{
			name:     ":nth-child with of clause",
			selector: ":nth-child(2n+1 of .active, #hero)",
			expected: theme.Specificity{A: 1, B: 1, C: 0}, // (0, 1, 0) + max((0, 1, 0), (1, 0, 0)) = (1, 1, 0)
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			actual := theme.ComputeSpecificity(tt.selector)
