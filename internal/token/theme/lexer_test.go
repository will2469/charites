...
func TestLexer_CSSEscapes(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input:         `-10px`,
			expectedType:  theme.TokenDimension,
			expectedValue: `-10px`,
			unescaped:     `-10px`,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			lexer := theme.NewLexer([]byte(tt.input))
			tok := lexer.NextToken()
			...
			unescaped := theme.UnescapeCSS(tok.Value)
