...
// TestBrowserRules_CanonicalContract menguji kontrak kanonikal untuk seluruh rule di kategori browser:
// 1. Format ID kanonikal (browser.<slug>)
// 2. Kategori wajib "browser"
...
func TestBrowserRules_CanonicalContract(t *testing.T) {
	...
	if len(browserRules) == 0 {
		t.Fatalf("no browser rules registered in builtin registry")
	}

	for _, rule := range browserRules {
		t.Run(rule.ID(), func(t *testing.T) {
			// 1. ID check
			if !strings.HasPrefix(rule.ID(), "browser.") || len(rule.ID()) <= 8 {
				t.Errorf("rule ID %q does not conform to canonical format 'browser.<slug>'", rule.ID())
			}

			// 2. Category check
			if rule.Category() != "browser" {
				t.Errorf("rule %s has category %q, want 'browser'", rule.ID(), rule.Category())
			}
