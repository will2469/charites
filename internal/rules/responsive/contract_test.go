...
// TestResponsiveRules_CanonicalContract menguji kontrak kanonikal untuk seluruh rule di kategori responsive:
// 1. Format ID kanonikal (responsive.<slug>)
// 2. Kategori wajib "responsive"
...
func TestResponsiveRules_CanonicalContract(t *testing.T) {
	reg := rules.NewRegistry()
	if err := rules.RegisterBuiltinRules(reg); err != nil {
		t.Fatalf("failed to register builtin rules: %v", err)
	}

	respRules := getResponsiveRules(reg)
	if len(respRules) == 0 {
		t.Fatalf("no responsive rules registered in builtin registry")
	}
...
func getResponsiveRules(reg *rules.Registry) []rules.Rule {
	var res []rules.Rule
	for _, r := range reg.All() {
		if r.Category() == "responsive" {
			res = append(res, r)
		}
	}
	return res
...
func assertRuleBasics(t *testing.T, rule rules.Rule) {
	if !strings.HasPrefix(rule.ID(), "responsive.") || len(rule.ID()) <= 11 {
		t.Errorf("rule ID %q does not conform to canonical format 'responsive.<slug>'", rule.ID())
	}
	if rule.Category() != "responsive" {
		t.Errorf("rule %s has category %q, want 'responsive'", rule.ID(), rule.Category())
	}
