...
// TestA11yRules_CanonicalContract menguji kontrak kanonikal untuk seluruh rule di kategori a11y:
// 1. Format ID kanonikal (<category>.<slug>)
// 2. Kategori wajib "a11y"
...
func TestA11yRules_CanonicalContract(t *testing.T) {
	reg := rules.NewRegistry()
	if err := rules.RegisterBuiltinRules(reg); err != nil {
		t.Fatalf("failed to register builtin rules: %v", err)
	}

	a11yRules := make([]rules.Rule, 0)
	for _, r := range reg.All() {
		if r.Category() == "a11y" {
			a11yRules = append(a11yRules, r)
		}
	}

	if len(a11yRules) == 0 {
		t.Fatalf("no a11y rules registered in builtin registry")
	}

	for _, rule := range a11yRules {
		t.Run(rule.ID(), func(t *testing.T) {
			// 1. ID check
			if !strings.HasPrefix(rule.ID(), "a11y.") || len(rule.ID()) <= 5 {
				t.Errorf("rule ID %q does not conform to canonical format 'a11y.<slug>'", rule.ID())
			}

			// 2. Category check
			if rule.Category() != "a11y" {
				t.Errorf("rule %s has category %q, want 'a11y'", rule.ID(), rule.Category())
			}
