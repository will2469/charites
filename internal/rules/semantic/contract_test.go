...
// TestSemanticRules_CanonicalContract menguji kontrak kanonikal untuk seluruh rule di kategori semantic:
// 1. Format ID kanonikal (semantic.<slug>)
// 2. Kategori wajib "semantic"
...
func TestSemanticRules_CanonicalContract(t *testing.T) {
	...
	if len(semanticRules) == 0 {
		t.Fatalf("no semantic rules registered in builtin registry")
	}

	for _, rule := range semanticRules {
		t.Run(rule.ID(), func(t *testing.T) {
			// 1. ID check
			if !strings.HasPrefix(rule.ID(), "semantic.") || len(rule.ID()) <= 9 {
				t.Errorf("rule ID %q does not conform to canonical format 'semantic.<slug>'", rule.ID())
			}

			// 2. Category check
			if rule.Category() != "semantic" {
				t.Errorf("rule %s has category %q, want 'semantic'", rule.ID(), rule.Category())
			}
