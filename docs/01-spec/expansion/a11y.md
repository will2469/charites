...
- **Tujuan:** Menjamin tombol, tautan, dan kontrol interaktif memiliki ukuran fisik minimum $\ge 44\times 44\text{px}$ ($2.75\text{rem}$) di layar mobile (Apple HIG / WCAG 2.5.8).
...
- **In-Scope:** Tag `<button>`, `<a>`, `<input>`, `<select>` dengan kelas Tailwind yang menghasilkan ukuran $< 44\text{px}$ tanpa padding kompensasi atau minimum dimensions.
...
- **Tujuan:** Mencegah aktivasi keliru (*miss-tap*) antara kontrol yang berdekatan dengan menjamin jarak antar kontrol $\ge 8\text{px}$ (`gap-2`).
...
- **In-Scope:** Sibling tombol atau elemen interaktif dengan `gap-0`, `gap-1`, atau margin $< 8\text{px}$.
- **Bad:**
  ```tsx
  <div className="flex gap-1">
    <button className="h-11 px-3 bg-destructive text-destructive-foreground">Hapus</button>
...
- **Tujuan:** Mencegah browser mobile Safari iOS melakukan auto-zoom paksa saat input berukuran font $< 16\text{px}$ difokuskan, yang merusak layout dan pengalaman mengetik.
...
- **Tujuan:** Menjamin kontrol isian formulir memiliki ketinggian ergonomis ($\ge 42\text{px}$) dan bantalan vertikal memadai (`py-2.5` atau `h-11`) agar teks dan font descender tidak terhimpit border.
...
- **In-Scope:** Kontrol input dengan padding vertikal kerdil (`p-1`, `py-1`, `h-7`, `h-8`) yang membuat input terjepit tanpa kompensasi `min-h-11`.
- **Bad:** `<input className="h-8 px-2 text-sm border" />`
- **Good (Tailwind v4):** `<input className="h-11 px-3.5 py-2.5 text-base sm:text-sm border rounded-lg" />`
- **Engine:** Token Geometry AST.
- **Severity:** Warning.
...
- **Mengapa Lolos Linter Standar:** `outline-none` / `outline-hidden` adalah kelas Tailwind yang valid. Linter biasa tidak memeriksa apakah deklarasi tersebut dipasangkan dengan cincin fokus pengganti (`focus-visible:ring-*`).
- **In-Scope:** Elemen interaktif dengan kelas `outline-none`, `outline-hidden`, atau `focus:outline-none` tanpa `focus-visible:ring-*`.
- **Bad:** `<button className="outline-none bg-primary text-primary-foreground">Simpan</button>`
- **Good (Tailwind v4):** `<button className="outline-hidden focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-primary text-primary-foreground">Simpan</button>`
...
|---|---|---|---|---|
| `a11y.touch-target-size` | Ukuran target sentuh $\ge 44\text{px}$ ($2.75\text{rem}$) | Butuh komputasi box model dari utility Tailwind | warning | Token Geometry AST |
| `a11y.touch-target-spacing` | Jarak aman miss-tap $\ge 8\text{px}$ (`gap-2`) | Butuh analisis spasial relasional antar sibling | warning | Relational AST |
...
| `a11y.input-cramped-padding` | Ketinggian ergonomis input $\ge 42\text{px}$ (`h-11`/`py-2.5`) | Butuh kalkulasi total height line + padding | warning | Token Geometry AST |
