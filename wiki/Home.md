...
| `theme.hardcode-opacity-color` | `theme` | `ERROR` | Detects utility classes with hardcoded or uncalibrated slash opacity modifiers bypassing global.css SSOT | [`theme.hardcode-opacity-color`](theme.hardcode-opacity-color) |
...
```mermaid
flowchart TD
    subgraph GoldenCorpus ["Canonical 1-SSOT Tri-Corpus Architecture (tests/correctness/<rule_id>/)"]
        subgraph P ["Positive Corpus (positive/) - Verified True Positives"]
        ...
        subgraph N ["Negative Corpus (negative/) - Zero False Positives"]
        ...
        subgraph A ["Adversarial Corpus (adversarial/) - Anti-Evasion Resilience"]
...
```
   ...
   - **Positive Fixtures (P1-P5):** Obvious, indirect, helper-wrapped, deeply nested, and aliased violations. Verified to trigger diagnostics at exact lines and column spans.
   - **Negative Fixtures (N1-N5):** Valid tokens, explicit ignore directives, third-party libraries, standard HTML, and untokenized custom values (the Banana Test). Verified to produce zero diagnostics.
   - **Adversarial Fixtures (A1-A7):** Template literal interpolations, ternary conditionals, spread props, dynamic classes, variable shadowing, and cyclic references. Verified to prevent bypass.
