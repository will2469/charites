# theme.hardcode-opacity-color: Avoid Hardcoded Opacity Colors with Semantic Replacements
...
The `theme.hardcode-opacity-color` rule enforces strict adherence to centralized semantic design tokens by forbidding ad-hoc slash opacity modifiers on base semantic colors (such as `bg-primary/10`, `text-primary/20`, or `border-destructive/20`) when an officially curated semantic replacement token exists in the design system.
...
> **Note:** Format codes like `txx` or `axx` are strictly rejected. Always use the canonical Semgrep identifier: `theme.hardcode-opacity-color`.
