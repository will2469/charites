...
flowchart TD
    ...
    IgnoreCheck -- "Not Ignored" --> Diag["4. Emit Diagnostic: responsive.desktop-only-content"]
```

### Step-by-Step Evaluation:
1. **AST Traversal:** Visits normalized `ir.Node` structures during AST streaming.
2. **Invariant Check:** Compares node attributes against rule invariants.
3. **Directive Suppression Check:** Honors inline `charites:ignore responsive.desktop-only-content` directives.
...
This rule is rigorously tested and validated across the canonical **1-SSOT Tri-Corpus** in `tests/correctness/responsive.desktop-only-content/`:

```mermaid
flowchart TD
    subgraph GoldenCorpus ["1-SSOT Tri-Corpus Test Matrix for responsive.desktop-only-content"]
        subgraph P ["Positive Corpus (tests/correctness/responsive.desktop-only-content/positive/)"]
        ...
        subgraph N ["Negative Corpus (tests/correctness/responsive.desktop-only-content/negative/)"]
        ...
        subgraph A ["Adversarial Corpus (tests/correctness/responsive.desktop-only-content/adversarial/)"]
