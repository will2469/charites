...
flowchart TD
    ...
    IgnoreCheck -- "Not Ignored" --> Diag["4. Emit Diagnostic: browser.chrome-only-api"]
```

### Step-by-Step Evaluation:
1. **AST Traversal:** Visits normalized `ir.Node` structures during AST streaming.
2. **Invariant Check:** Compares node attributes against rule invariants.
3. **Directive Suppression Check:** Honors inline `charites:ignore browser.chrome-only-api` directives.
...
This rule is rigorously tested and validated across the canonical **1-SSOT Tri-Corpus** in `tests/correctness/browser.chrome-only-api/`:

```mermaid
flowchart TD
    subgraph GoldenCorpus ["1-SSOT Tri-Corpus Test Matrix for browser.chrome-only-api"]
        subgraph P ["Positive Corpus (tests/correctness/browser.chrome-only-api/positive/)"]
        ...
        subgraph N ["Negative Corpus (tests/correctness/browser.chrome-only-api/negative/)"]
        ...
        subgraph A ["Adversarial Corpus (tests/correctness/browser.chrome-only-api/adversarial/)"]
