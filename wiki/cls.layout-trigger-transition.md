...
flowchart TD
    ...
    IgnoreCheck -- "Not Ignored" --> Diag["4. Emit Diagnostic: cls.layout-trigger-transition"]
```

### Step-by-Step Evaluation:
1. **AST Traversal:** Visits normalized `ir.Node` structures during AST streaming.
2. **Invariant Check:** Compares node attributes against rule invariants.
3. **Directive Suppression Check:** Honors inline `charites:ignore cls.layout-trigger-transition` directives.
...
This rule is rigorously tested and validated across the canonical **1-SSOT Tri-Corpus** in `tests/correctness/cls.layout-trigger-transition/`:

```mermaid
flowchart TD
    subgraph GoldenCorpus ["1-SSOT Tri-Corpus Test Matrix for cls.layout-trigger-transition"]
        subgraph P ["Positive Corpus (tests/correctness/cls.layout-trigger-transition/positive/)"]
        ...
        subgraph N ["Negative Corpus (tests/correctness/cls.layout-trigger-transition/negative/)"]
        ...
        subgraph A ["Adversarial Corpus (tests/correctness/cls.layout-trigger-transition/adversarial/)"]
