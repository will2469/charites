...
flowchart TD
    ...
    IgnoreCheck -- "Not Ignored" --> Diag["4. Emit Diagnostic: pwa.service-worker-missing"]
```

### Step-by-Step Evaluation:
1. **AST Traversal:** Visits normalized `ir.Node` structures during AST streaming.
2. **Invariant Check:** Compares node attributes against rule invariants.
3. **Directive Suppression Check:** Honors inline `charites:ignore pwa.service-worker-missing` directives.
...
This rule is rigorously tested and validated across the canonical **1-SSOT Tri-Corpus** in `tests/correctness/pwa.service-worker-missing/`:

```mermaid
flowchart TD
    subgraph GoldenCorpus ["1-SSOT Tri-Corpus Test Matrix for pwa.service-worker-missing"]
        subgraph P ["Positive Corpus (tests/correctness/pwa.service-worker-missing/positive/)"]
        ...
        subgraph N ["Negative Corpus (tests/correctness/pwa.service-worker-missing/negative/)"]
        ...
        subgraph A ["Adversarial Corpus (tests/correctness/pwa.service-worker-missing/adversarial/)"]
